...
entries with a future version (written by a newer apte) and warn once
per version.
...
from apte.history import storage
...
class TestFutureVersionSkipped:
    def test_future_version_is_skipped_by_load_history(self, tmp_path: Path) -> None:
        path = tmp_path / ".apte" / storage.HISTORY_FILE
        _write_jsonl(
            path,
            [
        ...
        with warnings.catch_warnings():
            ...
            entries = storage.load_history(history_dir=tmp_path / ".apte")
    ...
    def test_future_version_is_skipped_by_load_previous_run(
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            entry = storage.load_previous_run(history_dir=tmp_path / ".apte")
    ...
    def test_warning_raised_once_per_future_version(self, tmp_path: Path) -> None:
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            storage.load_history(history_dir=tmp_path / ".apte")
        future_warnings = [
