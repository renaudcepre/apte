...
from apte.evals.types import EvalSuiteReport
from apte.execution.capture import real_stdout
...
# done lazily inside methods so that `import apte` works without it;
...
class RichReporter(PluginBase):
    ...
    def on_test_fail(self, result: TestResult) -> None:
        ...
        if result.output:
            ...
            if len(lines) > max_lines:
                self._print(
                    f"[dim]       │ ... ({len(lines) - max_lines} more lines in .apte/last_run_stdout)[/]"
                )
        if result.is_eval:
    ...
    def on_session_complete(self, result: SessionResult) -> None:
        ...
        log_file = Path(".apte/last_run.log")
        stdout_file = Path(".apte/last_run_stdout")
        if log_file.exists() or stdout_file.exists():
            self._print(
                "Full output: [dim].apte/last_run.log, .apte/last_run_stdout[/]"
            )
