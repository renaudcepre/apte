...
def _is_future_schema(entry: dict[str, Any]) -> bool:
    """Return True if the entry was written by a newer apte version.
    ...
    if version not in _warned_future_versions:
        ...
        warnings.warn(
            ...
            f"but this apte supports up to {SCHEMA_VERSION}. "
            f"Those entries will be skipped. Upgrade apte to read them.",
            stacklevel=3,
        )
    return True
