"""Module loading utilities for Apte.

This module handles the infrastructure concern of loading Python modules
and extracting ApteSession instances from them.
"""
...
def load_session(target: str, app_dir: str = ".") -> ApteSession:
    """Load a ApteSession from a module path.
    ...
    Returns:
        The loaded ApteSession instance.

    Raises:
        LoadError: If the target format is invalid, module not found,
                   session not found, or session is not a ApteSession.
    """
    if ":" not in target:
    ...
    if not isinstance(session, ApteSession):
        raise LoadError(f"'{session_name}' is not a ApteSession")
