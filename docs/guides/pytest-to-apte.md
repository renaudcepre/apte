# Migrating from pytest to Apte

This guide helps you migrate existing pytest test suites to Apte. It covers the key conceptual differences and provides practical translation patterns.
...
```python
# Apte - scope determined by binding
from apte import fixture, ApteSession, ApteSuite

session = ApteSession()
suite = ApteSuite("MyTests")
...
```python
# Apte - built-ins
from apte import tmp_path, caplog, mocker, Mocker, Use
...
# Run filtered: apte run module:session --tags slow
# Exclude: apte run module:session --tags "not slow"
```
...
# Unit tests
users_suite = ApteSuite("Users", tags=["unit"])
orders_suite = ApteSuite("Orders", tags=["unit"])

# Integration tests
integration_suite = ApteSuite("Integration", tags=["integration"])
...
```python
# OK in Apte - creating a mock to pass as argument
...
# Apte
apte run myapp.tests.session:session
apte run myapp.tests.session:session --suite Users
apte run myapp.tests.session:session --tags "unit and not slow"
apte run myapp.tests.session:session --lf
```

## Migration Checklist

1. **Create a session file** with `ApteSession()`
2. **Convert fixtures to Apte syntax** (`@fixture`, `@factory()`)
...
8. **Update CI/CD** to use `apte run` command
9. 